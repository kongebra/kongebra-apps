...
// I prod deklareres strømmen av gitops (NATS helm, arbeidspakke 0.5);
// competition ensure-r den idempotent lokalt så fallback-oppsettet (compose +
// go run) og Aspire fungerer uten manuelt stream-oppsett.
const StreamName = "tl"

// platformTenantData er det competition konsumerer fra platform sine
// tenant-events (kun feltet den trenger for read-gaten).
// ponytail: kun public_visibility nå. Utvid når competition trenger mer av
// tenant (f.eks. slug for validering).
type platformTenantData struct {
	PublicVisibility *bool `json:"public_visibility"`
}

// tenantEventSubjects er platform-subjectene competition lytter på for å holde
...
func runTenantConsumer(ctx context.Context, js jetstream.JetStream, pool *pgxpool.Pool) error {
	cons, err := js.CreateOrUpdateConsumer(ctx, StreamName, jetstream.ConsumerConfig{
		Durable:        "competition-tenant-projection",
