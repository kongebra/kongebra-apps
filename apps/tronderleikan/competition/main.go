// competition-tjenesten (SPEC §2, §3, §7): eier Tournaments, Games, påmelding/
// deltakelse, lag og plasseringsresultater. Deltakelse peker på Person/Team som
// VERDI (aldri cross-service FK, SPEC §8). Person-referanser valideres synkront
// mot roster ved skriving (SPEC §7); alt annet er event-drevet. competition
// publiserer domene-events fra dag 1 (SPEC §9) selv om ingen lytter ennå.
package main
...
func run() error {
	...
	if os.Getenv("OTEL_EXPORTER_OTLP_ENDPOINT") != "" {
		shutdownOTel, err = otelsetup.Setup(ctx, "tronderleikan-competition")
		if err != nil {
	...
	a := &api{
		...
		validator: validator,
		roster:    NewRosterClient(cfg.RosterURL),
	}
	...
	go func() {
		log.Printf("tronderleikan-competition listening on :%s (version=%s)", cfg.Port, v)
