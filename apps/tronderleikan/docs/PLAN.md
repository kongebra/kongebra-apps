...
|---|---|---|---|---|---|
| 0.1 | Mappestruktur + pkg-lib | `apps/tronderleikan/`-tre, `go.work`, `pkg/` med event-envelope, outbox-writer/publisher, JWT-validering (JWKS fra env-issuer), tenant-context (inkl. `SET LOCAL app.tenant_id`), otel-oppsett. Enhetstester | §5, §8, §9, §11 | - | TODO |
...
| 1.2 | roster-tjenesten | Person CRUD, manuell account-kobling, person-events | §4, §7, §8 | 0.x | TODO |
