-- +goose Up
-- Tenant-projeksjon: lokal read-model av tenant-fakta roster trenger, men ikke
-- eier. platform-tjenesten (SPEC §7) eier tenant-registeret; roster lærer
-- public_visibility via platform sine tenant-events (SPEC §7: "refs valideres
-- ved skriving, ellers events"). Konsumenten (consumer.go) upserter denne fra
-- tl.platform.tenant.provisioned|updated.
--
